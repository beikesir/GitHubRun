...
class MessageAdapter(
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        if (msg.batchId.isNotBlank()) {
            bindBatch(holder, batchGroupAt(position))
        } else if (msg.locked) {
    ...
    private fun bindBatch(holder: VH, group: List<Message>) {
        ...
        if (isExpanded) {
            ...
            // 折叠：只铺一行，第三格半宽表示「后面还有」；
            // 若总数超过三，再在半格上叠一层角标提示剩余数量
            val badge = if (group.size > perRow) "+${group.size - 2}" else null
            holder.batchGrid.addView(
                buildRow(ctx, group.take(perRow), halfLast = group.size > 2, badge = badge)
            )
        }
            ...
            val wrapped = if (isHalf && !badge.isNullOrEmpty()) {
                FrameLayout(ctx).apply {
                    ...
                    addView(TextView(ctx).apply {
                        ...
                        background = roundedBg(ctx, "#000000").apply { alpha = 120 }
