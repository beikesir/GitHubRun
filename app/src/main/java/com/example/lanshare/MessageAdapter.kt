...
class MessageAdapter(
     ...
     *
     * v5.6 起统一为「以宽度为准」的一套规则：
     *   头像边长           界面 1/9
     *   标准宽度 STD_W     界面 9/20  —— 横图与方图的显示宽度
     *   竖图默认宽度        界面 11/40 —— 竖图的显示宽度
     *   长边上限 MAX_LEN   STD_W 的 2 倍 —— 竖图算出的高度超过它时改以它为高
     *   文件卡片           宽 = 界面 3/5，高 = 界面 1/6
     */
    ...
    // ==================== 显示尺寸（全部由界面宽度换算） ====================
    /** 头像边长：界面 1/9 */
    private val avatarPx = (uiWidthPx / 9f).toInt().coerceIn(20, 140)

    /** 标准宽度：横图与方图的显示宽度 = 界面 9/20 */
    private val stdWPx = (uiWidthPx * 9f / 20f).toInt().coerceAtLeast(40)

    /** 竖图默认显示宽度 = 界面 11/40 */
    private val portWPx = (uiWidthPx * 11f / 40f).toInt().coerceAtLeast(24)

    /** 长边上限 = 标准宽度的 2 倍：竖图算出的高度超过它时改以它为高 */
    private val maxLenPx = stdWPx * 2

    /** 文件卡片：宽 = 界面 3/5、高 = 界面 1/6 */
    private val fileWPx = (uiWidthPx * 3f / 5f).toInt().coerceAtLeast(80)
    private val fileHPx = (uiWidthPx / 6f).toInt().coerceAtLeast(24)

    /** 文字气泡最大宽度：留出头像与两侧边距后剩下的空间 */
    private val textMaxPx = (uiWidthPx * 0.62f).toInt().coerceAtLeast(120)
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        when {
                ...
                msg.isImage -> {
                    ...
                    holder.fileCard.visibility = View.GONE
                    holder.batchBox.visibility = View.GONE
                    // 横图/方图以标准宽（界面 9/20）为准，竖图宽 = 界面 11/40，
                    // 长边超过标准宽 2 倍时改以上限为准
                    applyImageSize(holder.ivImage, msg, ctx, msg.isMe)
    ...
    private fun bindRowGroup(holder: VH, group: List<Message>) {
        ...
        holder.row.layoutParams = lp
        layoutRow(holder, ctx, head.isMe)
     ...
     *              用来暗示「下面还有」，而不是把图片压扁。
     */
    private fun buildImageCell(ctx: Context, m: Message, full: Boolean, me: Boolean): View {
        val g = if (me) Gravity.END else Gravity.START
        return if (full) {
            ImageView(ctx).apply {
                // 与单张图片同一套规则：横图/方图以标准宽为准，竖图宽 = 界面 11/40
                val sz = probeSize(m, ctx)
                if (sz == null) {
                    adjustViewBounds = true
                    layoutParams = LinearLayout.LayoutParams(
                        stdWPx, LinearLayout.LayoutParams.WRAP_CONTENT
                    ).apply { bottomMargin = dp(ctx, 3); gravity = g }
                } else {
                    val (w, h) = displaySize(sz.first, sz.second)
                    adjustViewBounds = false
                    layoutParams = LinearLayout.LayoutParams(w, h).apply {
                        bottomMargin = dp(ctx, 3); gravity = g
                    }
                }
     ...
     */
    private fun buildHalfCell(ctx: Context, m: Message, me: Boolean): View {
        // 容器高度取「该图完整显示高度」的一半。
        // 完整尺寸由 displaySize 按新规则算出（横图/竖图规则不同，不能写死）。
        val sz = probeSize(m, ctx)
        val (fullW, fullH) = if (sz != null) displaySize(sz.first, sz.second)
        else stdWPx to stdWPx
        val container = FrameLayout(ctx).apply {
            layoutParams = LinearLayout.LayoutParams(
                fullW, (fullH / 2).coerceAtLeast(dp(ctx, 20))
            ).apply {
    ...
    /**
     * 按横竖比例算出图片的显示宽高（两端同一套规则）。
     *
     *   横图 / 方图（宽 >= 高）以宽为准：宽 = STD_W，高按原比例（必然 <= 宽）。
     *   竖图（宽 <  高）以宽为准：宽 = 界面 11/40，高按原比例；
     *       但长边（此处是高度）不能超过 STD_W 的 2 倍，
     *       超过则以该上限为高、宽度按比例回算——避免长截图撑满整屏。
     */
    private fun displaySize(w0: Int, h0: Int): Pair<Int, Int> {
        if (w0 <= 0 || h0 <= 0) return stdWPx to stdWPx
        return if (w0 >= h0) {
            val w = stdWPx
            val h = (w * h0f(w0, h0)).roundToInt().coerceAtLeast(1)
            w to h
        } else {
            var w = portWPx
            var h = (w * h0f(w0, h0)).roundToInt().coerceAtLeast(1)
            if (h > maxLenPx) {
                h = maxLenPx
                w = (h * w0.toFloat() / h0).roundToInt().coerceAtLeast(1)
            }
            w to h
        }
    }
    ...
    /** 高宽比（高 / 宽），避免上面重复写浮点转换 */
    private fun h0f(w0: Int, h0: Int) = h0.toFloat() / w0

    /**
     * 给图片定尺寸。
     * 已知原图比例时直接给出确定的宽高（最稳，不会出现空白占位）；
     * 比例未知（例如内容还在中继上、尚未落到本机）时退化为
     * 「按标准宽定宽 + adjustViewBounds 自适应」，至少不会裁切或拉伸。
     */
    private fun applyImageSize(iv: ImageView, m: Message, ctx: Context, me: Boolean) {
        val g = if (me) Gravity.END else Gravity.START
        val sz = probeSize(m, ctx)
        if (sz == null) {
        ...
        iv.scaleType = ImageView.ScaleType.FIT_CENTER
        iv.layoutParams = LinearLayout.LayoutParams(w, h).apply { gravity = g }
    }
     ...
     * 摆放「头像 + 内容列」的顺序。
     *
     * 此前用 layoutDirection = RTL 让自己的消息头像排到右侧，
     * 但该行为在部分 ROM / 系统版本上并不一致，实际表现就是
     * 「头像与发送者名字的左右关系反了」。这里改为代码显式控制子视图顺序，
     * 结果完全确定，不再依赖 RTL 解析。
     */
    private fun layoutRow(holder: VH, ctx: Context, me: Boolean) {
        val gap = dp(ctx, 5)
        // 头像尺寸：界面 1/9，间距按方向落在「头像与内容之间」那一侧
        holder.ivAvatar.layoutParams = LinearLayout.LayoutParams(avatarPx, avatarPx).apply {
            topMargin = dp(ctx, 2)
            marginStart = if (me) gap else 0
        }
        (holder.bubble.layoutParams as? LinearLayout.LayoutParams)?.let {
            it.marginStart = if (me) 0 else gap
            holder.bubble.layoutParams = it
        }
        // 自己的消息：[内容][头像]（头像在外侧＝右）；他人：[头像][内容]
        val first: View = if (me) holder.bubble else holder.ivAvatar
        val second: View = if (me) holder.ivAvatar else holder.bubble
        val ok = holder.row.childCount >= 2 &&
            holder.row.getChildAt(0) === first && holder.row.getChildAt(1) === second
        if (!ok) {
            holder.row.removeAllViews()
            holder.row.addView(first)
            holder.row.addView(second)
        }
        alignContent(holder, me)
    }

    /** 内容列内部对齐：自己的消息整体靠右，他人靠左（与扩展端 .mc 一致） */
    private fun alignContent(holder: VH, me: Boolean) {
        val g = if (me) Gravity.END else Gravity.START
        setGravity(holder.tvSender, g)
        setGravity(holder.tvText, g)
        setGravity(holder.tvTime, g)
        setGravity(holder.fileCard, g)
        setGravity(holder.batchBox, g)
        holder.tvText.maxWidth = textMaxPx
    }

    private fun setGravity(v: View, g: Int) {
