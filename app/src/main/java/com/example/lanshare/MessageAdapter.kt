...
class MessageAdapter(
    ...
    /**
     * 折叠时露出的项数。露出 2 项 + 一个「展开其余 N 项」按钮，
     * 既不占太高，也足够表明这是一组图。
     */
    ...
    private fun bindBatch(holder: VH, group: List<Message>) {
        ...
        // v3.7：改为纵向堆叠——每项占满整行自上而下排列，展开即向下铺开。
        // 之前是「两个完整 + 半格」的横向裁切，角标含义绕、气泡也不紧凑。
        holder.batchGrid.removeAllViews()
        val show = if (isExpanded) group else group.take(collapsedShow)
        show.forEach { m -> holder.batchGrid.addView(buildStackCell(ctx, m)) }

        val rest = group.size - collapsedShow
        holder.batchToggle.text = when {
            group.size <= collapsedShow -> ""            // 无需展开，按钮留空
            isExpanded -> "△ 收起"
            else -> "▽ 展开其余 " + rest + " 项"
        }
    ...
    /** 纵向堆叠中的一项：整行宽度的图片（或兜底的文件卡片） */
    private fun buildStackCell(ctx: Context, m: Message): View {
        val h = dp(ctx, 132)
