...
class WebSocketManager {
    ...
    /**
     * 发送图片（原图直传，不再压缩）。
     * @param viaShare 是否由系统分享触发；为 true 时接收端会自动落盘
     */
    ...
    private fun dispatchSent(type: String, content: String, meta: FileMeta? = null, source: String = "") {
        val incoming = Incoming(
            type = type,
            content = content,
    ...
    /**
     * 返回值表示「是否受理」，而非「是否已即时送达」。
     *
     * v3.7：此前未连上时入队却返回 false，界面据此弹「未连接，将自动重试」，
     * 但消息其实已在队列里、连上后照常补发——用户看到的就是
     * 「提示未连接、文件却传过去了」。现在入队成功即视为受理，不再误报。
     * 仅当用户主动断开（userClosed）才真的拒绝。
     */
    private fun sendRaw(msg: String): Boolean {
        if (ready && ws != null) return ws!!.send(msg)
        if (userClosed) return false
        synchronized(pending) { pending.add(msg) }
        scheduleReconnect()
        if (BuildConfig.DEBUG) Log.d(TAG, "未连上，已入队待发（${msg.length} 字节）")
        return true
    }
