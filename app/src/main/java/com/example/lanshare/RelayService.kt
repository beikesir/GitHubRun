    ...
    private val msgListener: MsgListener = { msg ->
        ...
        when (msg.type) {
            "image" -> {
                ...
                val saved = if (autoSave) saveImageToGallery(msg.content) else null
                notifyMsg("[图片]",
                    saved?.let { "已保存到相册：$it" } ?: "收到一张图片（长按可下载）")
            }
            "file" -> {
                // 文件落到公共下载目录，保留原始文件名
                val saved = if (autoSave && msg.content.isNotBlank())
                    FileSaver.saveDataUrl(this, msg.content, msg.fileName, msg.mime) else null
