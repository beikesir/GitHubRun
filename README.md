...
### v3.6
- **浏览器端：文件选择器不再卡住**。原先点发送会开一个小窗口并在其中调用 `input.click()`，但 `chrome.windows.create` 打开的新窗口不继承用户手势，浏览器会静默忽略这次点击，界面就永远停在"正在打开文件选择器"。现改为在面板内直接触发系统选择器，手势完整，且少一层界面；选完立即交给后台发送。
- **浏览器端：批量消息合并不再依赖相邻**。此前只有连续的同批消息才会聚合，一旦这批消息之间插入其他消息就会散成多个气泡。现按 `batchId` 全局归并，并按 `batchIndex` 还原顺序。
- **下载改为右键触发**：单击图片是看大图，单击文件不再落盘；保存统一走右键菜单，避免误触。
- **安卓端：批量气泡收紧**。只有两项以上才折叠（单张按普通消息渲染，不再套空壳）；折叠态固定为「两个完整 + 第三格半宽」，角标含义改为"还有几项未露出"；新增紧凑气泡背景（内边距 12/8dp → 5dp），网格尺寸与气泡上限重新配平。
- 三端版本号已统一：中继 `v3.6`／扩展 `3.6.0`／安卓 `3.6`（versionCode 14）。

### v3.5
- 修复安卓端编译错误：`MessageAdapter` 中 `TextView.singleLine` 无对应 getter、Kotlin 无法合成属性，改用 `maxLines = 1`。
- 发布前静态校验：属性赋值声明、资源引用（0 缺失）、XML 语法（28 文件）、跨文件符号（0 缺失）均通过。
- 三端版本号已统一：中继 `v3.5`／扩展 `3.5.0`／安卓 `3.5`（versionCode 13）。
